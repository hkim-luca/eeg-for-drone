...
# === LOCAL_BCI2A_GDF_ADAPTER_V1 ===
def _local_find_eval_label(project_root, subject):
    from pathlib import Path
    base = Path(project_root) / "data" / "BCI_IV_2a"
    candidates = [
        base / "true_labels" / f"{subject}E.mat",
        base / "labels" / f"{subject}E.mat",
        base / "evaluation_labels" / f"{subject}E.mat",
        base / "raw" / f"{subject}E.mat",
        base / f"{subject}E.mat",
    ]
    candidates += list(base.rglob(f"{subject}E.mat"))
    for p in candidates:
        if p.exists(): return p
    raise FileNotFoundError(f"Official evaluation label not found for {subject}: {candidates}")

def _local_load_one_gdf(gdf_path, subject, session, fmin, fmax, project_root):
    import mne
    import numpy as np
    from scipy.io import loadmat
    raw = mne.io.read_raw_gdf(gdf_path, preload=True, verbose=False)
    picks = mne.pick_types(raw.info, meg=False, eeg=True, eog=False, stim=False, exclude=[])
    picks = picks[:22] if len(picks) >= 22 else np.arange(22)
    raw.pick(picks)
    raw.filter(l_freq=fmin, h_freq=fmax, method="fir", phase="zero", verbose=False)
    # Keep native 250 Hz. Public BCIDataset later keeps the first 1000 samples.
    events, mapping = mne.events_from_annotations(raw, verbose=False)
    if session == "T":
        wanted = ["769", "770", "771", "772"]
        codes=[]; code_to_label={}
        for label,key in enumerate(wanted):
            hit=next((k for k in mapping if str(k).strip()==key or str(k).strip().endswith(key)),None)
            if hit is None: raise ValueError(f"{subject}T missing event {key}; available={mapping}")
            code=mapping[hit];codes.append(code);code_to_label[code]=label
        selected=events[np.isin(events[:,2],codes)]
        y=np.asarray([code_to_label[int(v)] for v in selected[:,2]],dtype=np.int64)
    else:
        hit=next((k for k in mapping if str(k).strip()=="783" or str(k).strip().endswith("783")),None)
        if hit is None: raise ValueError(f"{subject}E missing event 783; available={mapping}")
        code=mapping[hit];selected=events[events[:,2]==code]
        mat=loadmat(_local_find_eval_label(project_root,subject))
        if "classlabel" in mat: y=np.asarray(mat["classlabel"]).squeeze()
        else:
            keys=[k for k in mat if not k.startswith("__") and np.asarray(mat[k]).size==len(selected)]
            if len(keys)!=1: raise KeyError(f"Cannot identify labels for {subject}E: {list(mat)}")
            y=np.asarray(mat[keys[0]]).squeeze()
        y=y.astype(np.int64)
        if y.min()==1: y=y-1
    # Cue event is the origin. Epoch 0..4 s gives 1000 samples at 250 Hz.
    epochs=mne.Epochs(raw,selected,event_id=None,tmin=0.0,tmax=4.0-1.0/raw.info["sfreq"],baseline=None,
                      preload=True,proj=False,reject_by_annotation=False,verbose=False)
    X=epochs.get_data(copy=True).astype(np.float32)
    if len(X)!=len(y): raise ValueError(f"{subject}{session}: X={len(X)} labels={len(y)}")
    if X.shape[1:]!=(22,1000): raise ValueError(f"{subject}{session}: unexpected shape {X.shape}")
    return X,y

def load_dataset(name="iv2a", fmin=0.5, fmax=100.0):
    """Local-GDF replacement for the public MOABB load_dataset function."""
    import os
    import numpy as np
    import pandas as pd
    from pathlib import Path
    if name != "iv2a": raise ValueError("Local adapter currently supports iv2a only")
    project_root=Path(os.environ.get("LOCAL_BCI_PROJECT_ROOT", Path(__file__).resolve().parents[2]))
    raw_dir=project_root/"data"/"BCI_IV_2a"/"raw"
    xs=[];ys=[];meta=[]
    for i in range(1,10):
        subject=f"A{i:02d}"
        for session in ("T","E"):
            path=raw_dir/f"{subject}{session}.gdf"
            if not path.exists(): raise FileNotFoundError(path)
            X,y=_local_load_one_gdf(path,subject,session,fmin,fmax,project_root)
            xs.append(X);ys.append(y)
            meta.extend({"subject":i,"session":session,"run":f"{session}_0"} for _ in range(len(y)))
    X=np.concatenate(xs);y=np.concatenate(ys);meta=pd.DataFrame(meta)
    print(f"    Local GDF IV-2a: X={X.shape}, classes=4, channels=22, trials={len(y)}")
    print("    Sessions:",meta.groupby(["subject","session"]).size().to_dict())
    return X,y,meta,4,22
# === END_LOCAL_BCI2A_GDF_ADAPTER_V1 ===
