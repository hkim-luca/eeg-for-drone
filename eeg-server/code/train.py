...
def train_model(X_train, y_train, X_val, y_val, X_test, y_test, model_class, device,
                ...
                tta_div=1.0, n_classes=4, n_channels=22, checkpoint_path=None, checkpoint_metadata=None):
    """Leak-free training.
    ...
    if refit:
        if checkpoint_path is not None:
            raise ValueError('Checkpoint export is supported for refit=False only; LOSO reproduction uses refit=False.')
    ...
    # Save the validation-selected SOURCE model before any target-dependent TTA.
    # This is the portable checkpoint that a teammate should receive.
    if checkpoint_path is not None:
        from pathlib import Path
        checkpoint_path = Path(checkpoint_path)
        checkpoint_path.parent.mkdir(parents=True, exist_ok=True)
        payload = {
            'model_state_dict': {k: v.detach().cpu().clone() for k, v in final_model.state_dict().items()},
            ...
            'checkpoint_stage': 'validation_selected_source_model_before_tta',
            'checkpoint_metadata': checkpoint_metadata or {},
        }
        torch.save(payload, checkpoint_path)
        print(f"      Saved source checkpoint: {checkpoint_path}", flush=True)
