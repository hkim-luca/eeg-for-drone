...
def main() -> None:
    ...
    fig, (ax_raw, ax_band) = plt.subplots(2, 1, figsize=(10, 7))
    ...
    ax_band.set_title("EEG band power  +  Attention / Meditation")
    ax_band.grid(True, axis="y", alpha=0.3)
    state_txt = ax_band.text(
        0.99, 0.95, "", transform=ax_band.transAxes,
    ...
    def update(_frame: int) -> tuple[Artist, ...]:
        ...
        state_txt.set_text(
            f"Attention {state.attention:>3}   Meditation {state.meditation:>3}"
        )
        ...
        return (raw_line, quality_txt, state_txt, *bars)
