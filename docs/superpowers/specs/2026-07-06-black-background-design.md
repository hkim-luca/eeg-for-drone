...
- 옵션 제공 방식:
  - 실행 옵션 `-blackbg` : 시작부터 검은 배경으로 실행 (기존 `-recording` 패턴과 동일).
  - 런타임 토글 키 `B` : 실행 중 켜고 끄기 (기존 `P` 키 물리 설정 패턴과 동일).
...
- **`ADroneSimPlayerController`** (수정)
  ...
  - `BeginPlay` : `-blackbg` 파싱 시 다음 틱에 적용(폰 빙의 완료 후 폰이 제외되도록).
  - 멤버 `FEnvironmentBlackout EnvironmentBlackout`.

## 오류 처리 / 엣지 케이스

- 폰이 아직 없으면(빙의 전) 다음 틱 적용으로 회피.
- Restore 시 파괴된 액터는 weak pointer로 건너뜀.
- 시나리오 종료 후 레벨 리로드 시 상태는 초기화되며, `-blackbg` 실행 옵션이면
  새 컨트롤러 BeginPlay에서 자동 재적용.

## 테스트

- 기존 테스트는 순수 물리 로직 대상이라 월드 렌더링 토글은 단위 테스트 범위 밖.
- 검증: 게임 타깃 컴파일 + 사용자 실행 확인(`-blackbg`, B 키 토글, 위젯 표시,
  드론 조명 유지).
